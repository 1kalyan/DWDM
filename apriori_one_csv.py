# apriori_one_csv.py
# Run: put your CSV in the same folder (e.g., data.csv) then:
# python apriori_one_csv.py
import csv
import os
from collections import defaultdict
from itertools import combinations

CSV_FILENAME = "data.csv"  # change if your file has a different name
...
def load_transactions_from_csv(path):
    ...
    with open(path, newline="", encoding="utf-8") as f:
        reader = csv.reader(f)
        for row in reader:
            # supports either: multiple columns OR a single column "a, b, c"
...
if __name__ == "__main__":
    tx = load_transactions_from_csv(CSV_FILENAME)
    freq = apriori(tx, min_support=0.3)
    print("# Frequent Itemsets (Apriori, one CSV)")
    ...
    print("# Association Rules (Apriori, one CSV)")
