# fpgrowth_one_csv.py
# Run: put your CSV (e.g., data.csv) next to this file, then:
# python fpgrowth_one_csv.py
import csv
import os
...
# ---- minimal FP-growth helpers (same as sample, shortened comments) ----
class Node:
    def __init__(self, item, parent=None):
        self.item = item; self.count = 1; self.parent = parent
...
def mine(header, min_support, n, suffix, out):
    for item, (count, head) in sorted(header.items(), key=lambda kv: (kv[1][0], kv[0])):
        ...
        while cur is not None:
            path = ascend(cur)
            if path: cond_tx.extend([set(path)] * cur.count)
            cur = cur.link
        if cond_tx:
            _, cond_header, _ = build_tree(cond_tx, min_support)
            if cond_header: mine(cond_header, min_support, len(cond_tx), new_suf, out)
