# id3_multi_csv.py
# Run: python id3_multi_csv.py
# - Reads and stacks multiple CSVs (same columns)
# - Expects target column 'label' (change TARGET if needed)
...
# ---- (ID3 core reused in compact form) ----
class Node:
...
def entropy(y):
    ...
    vals,cnt=np.unique(y,return_counts=True); p=cnt/cnt.sum()
...
def information_gain(parent_y, splits):
    ...
    for s in splits: hc += (len(s)/total)*entropy(s)
    return H-hc
...
def best_numeric_threshold(x,y):
    ...
    for t in cand:
        yl=y[x<=t]; yr=y[x>t]; g=information_gain(y,[yl,yr])
        if g>best_g: best_g=g; best_t=t
    return best_t,best_g

def majority_label(y):
    v,c=np.unique(y,return_counts=True); return v[np.argmax(c)]
...
def id3_train(df,target_col="label",max_depth=None,min_samples_split=2):
    ...
    def build(SX,Sy,d):
        ...
        if (max_depth is not None and d>=max_depth) or len(Sy)<min_samples_split or SX.shape[1]==0:
        ...
        if best_num:
            ...
            node.children["<="]=build(SX[L].drop(columns=[best_f]),Sy[L],d+1)
            node.children[">"]=build(SX[R].drop(columns=[best_f]),Sy[R],d+1)
            return node
        else:
            ...
            for v in colv.dropna().unique():
                m=colv==v
                node.children[v]=build(SX[m].drop(columns=[best_f]),Sy[m],d+1)
            if colv.isna().any():
                m=colv.isna()
                node.children["<NA>"]=build(SX[m].drop(columns=[best_f]),Sy[m],d+1)
            return node
    return build(X,y,0)
...
if __name__ == "__main__":
    ...
    train_df, test_df = train_test_split_df(df, test_size=0.25, seed=7)
